...
async function decryptAESGCM(base64Data, secret) {
    const rawData = Uint8Array.from(atob(base64Data), c => c.charCodeAt(0));

    // Ambil IV (12 byte pertama), Tag (16 byte berikutnya), sisanya ciphertext
    const iv = rawData.slice(0, 12);
    const tag = rawData.slice(12, 28);
    const ciphertext = rawData.slice(28);

    const keyBytes = await getKey(secret);
    ...
    // GCM di Web Crypto: tag otomatis di-append di ciphertext
    // Jadi kita gabungkan ciphertext + tag
...
var _reloadMenu = function () {
  ...
  $.get('https://raw.githubusercontent.com/nafkid2023/web/main/json/livetv.json', {}, function (_0x4d5bc0) {
